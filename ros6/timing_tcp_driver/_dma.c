...
//  out32(addr+PLX9080_INTCSR,0xf0900);
  ...
  out8(addr+PLX9080_DMACSR1,0x0);       // brookes - this line overwrites the first?
  out32(addr+PLX9080_DMAMODE1,0x00801); 
  out32(addr+PLX9080_DMAPADR1,dmapadr); // brookes - what if an int is not 32 bits?
  out32(addr+PLX9080_DMALADR1,dmaladr); // brookes - what if an int is not 32 bits?
...
void DMA_readout(long addr,int dmapadr,int dmaladr,long dmasiz){
  ...
  out32(addr+PLX9080_DMAMODE1,0x00803); // brookes - only difference from above is 80_3_
